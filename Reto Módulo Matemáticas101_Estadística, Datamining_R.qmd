...
```{r}
bwt <- read.csv("~/BD15/Matematicas_101/estadistica-datamining/data/birthweight_reduced.csv")
head(bwt)
```
...
```{r}
non_smoker <- bwt$Birthweight[which(bwt$smoker=='No')] 
mean(non_smoker)
t.test(non_smoker)
```
...
```{r}
non_smoker <- bwt$Birthweight[which(bwt$smoker=='Yes')] 
mean(non_smoker)
t.test(non_smoker)
```
...
```{r}
probs <- 1-pbinom(4, size = 1000, p=p)
probs
```
...
```{r}
paste("La aparición sería entorno a",model2$coef[1])
```
